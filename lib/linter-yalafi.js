...
//TODO some more filters are needed for warnings
...
export default {
  activate() {
    ...
    this.subscriptions.add(atom.config.observe('linter-yalafi.disabledRules', (value) => {
      this.disabledRules = value;
    }));
    this.subscriptions.add(atom.config.observe('linter-yalafi.executablePath', (value) => {
      this.executablePath = value;
    }));
    this.subscriptions.add(atom.config.observe('linter-yalafi.yalafiOptions', (value) => {
      this.yalafiOptions = value;
    }));
    this.subscriptions.add(atom.config.observe('linter-yalafi.languageToolPath', (value) => {
      this.languageToolPath = value;
    }));
    this.subscriptions.add(atom.config.observe('linter-yalafi.languageToolnGramDataPath', (value) => {
      this.languageToolnGramDataPath = value;
    }));
    this.subscriptions.add(atom.config.observe('linter-yalafi.languageToolServerOptions', (value) => {
  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        args = args.concat(this.yalafiOptions);
        args.push(filePath);
        ...
        if (filteredErrors) {
          //TODO uncomment when error filtering is improved
